# Holistic decision analysis model of silvoarable nut agroforestry system of Hof Lebensberg ####
...
AF_benefit <- function(x, varnames)
{
  ...
  #Running cost (for explanations and units see "Running cost" under "AF costs")
  Treeless_maize_sowing_cost <- rep(0, n_years)
  ...
  Treeless_maize_yield <- rep(0, n_years)
  ...
  #Benefits treeless arable system ####
  
  # Arable system is managed with crop rotation of Maize(CCM)-Wheat-Barley-Rapeseed -> one crop is grown once every 4 years
  
  Treeless_maize_yield[maize_indices] <- vv(maize_yield, cv_maize_yield, length(maize_indices)) * arable_area_treeless
  Treeless_maize_benefit <- vv(maize_price, cv_maize_price, n_years) * Treeless_maize_yield
  ...
  Treeless_barley_benefit <- vv(barley_price, cv_barley_price, n_years) * Treeless_barley_yield
  ...
  Treeless_rapeseed_benefit <- vv(rapeseed_price, cv_rapeseed_price, n_years) * Treeless_rapeseed_yield
  
  # Cost of managing arable system ####
  
  #maize
  Treeless_maize_sowing_cost[maize_indices] <- vv(maize_seed_price, cv_maize_seed_price, length(maize_indices)) * arable_area_treeless #cost of seed [€/ha]*area managed [ha]
  ...
  Treeless_maize_labour_cost <- Maize_labour * arable_area_treeless * Labour_costs #Labour cost associated with maize cultivation in treeless arable system
  ...
  Treeless_barley_labour_cost <- Barley_labour * arable_area_treeless * Labour_costs #Labour cost associated with barley cultivation in treeless system
  ...
  Treeless_rapeseed_labour_cost <- Rapeseed_labour * arable_area_treeless * Labour_costs #Labour cost associated with rapeseed cultivation in treeless system
  ...
  # AF costs ####  
  #Implementation cost 
  Labour_costs <- rep(0, n_years) #Gross wages of farm employees (also applies to treeless system) [€]
  AF_planning_cost <- rep(0, n_years) #Invoice of service provider (planners/consultants), planning the AF system [€]
  AF_pruning_course <- rep(0, n_years) #Costs of the pruning training of an employee [€]
  ...
  AF_apple_harvest <- rep(0, n_years) #Labour cost of harvesting apples manually [€]
  ...
  AF_mowing_treerow <- rep(0, n_years) #Labour hours of mowing the tree rows manually [h/ha]
  
  #Cost of seeds of arable crops (Labour involved in sowing is separately listed) [€]
  AF_maize_sowing_cost <- rep(0, n_years)
  AF_wheat_sowing_cost <- rep(0, n_years)
  AF_barley_sowing_cost <- rep(0, n_years)
  AF_rapeseed_sowing_cost <- rep(0, n_years)
  #Cost of fertilizer needed for arable crops [€]
  AF_maize_fertilizer_cost <- rep(0, n_years)
  AF_wheat_fertilizer_cost <- rep(0, n_years)
  AF_barley_fertilizer_cost <- rep(0, n_years)
  AF_rapeseed_fertilizer_cost <- rep(0, n_years)
  #Cost of pesticides used in arable crops [€]
  AF_maize_pesticide_cost <- rep(0, n_years)
  AF_wheat_pesticide_cost <- rep(0, n_years)
  AF_barley_pesticide_cost <- rep(0, n_years)
  AF_rapeseed_pesticide_cost <- rep(0, n_years)
  #Cost for machinery used in arable crops (includes fixed and variable cost but not the investment) [€]
  AF_maize_machinery_cost <- rep(0, n_years)
  AF_wheat_machinery_cost <- rep(0, n_years)
  AF_barley_machinery_cost <- rep(0, n_years)
  AF_rapeseed_machinery_cost <- rep(0, n_years)
  #Cost for hail insurance needed for arable crops [€]
  AF_maize_insurance_cost <- rep(0, n_years)
  AF_wheat_insurance_cost <- rep(0, n_years)
  AF_barley_insurance_cost <- rep(0, n_years)
  AF_rapeseed_insurance_cost <- rep(0, n_years)
  ...
  #Hier muss ein Trend rein! Wie verändert sich der Mindestlohn über Zeit? Wie der zu erwartende Lohn bei landwirtschaftlichen Mitarbeitern?
  ...
  #Management cost of arable component ####
  ...
  #Maize
  AF_maize_sowing_cost[maize_indices] <- vv(maize_seed_price, cv_maize_seed_price, length(maize_indices)) * Arable_area_AF #cost of seed [€/ha]*area managed [ha]
  AF_maize_fertilizer_cost[maize_indices] <- vv(maize_fert_price, cv_maize_fert_price, length(maize_indices)) * Arable_area_AF #cost of fertilizer [€/ha]*area managed [ha]
  AF_maize_pesticide_cost[maize_indices] <- vv(maize_cides_price, cv_maize_cides_price, length(maize_indices)) * Arable_area_AF #cost of pesticides [€/ha]*area managed [ha]
  AF_maize_machinery_cost[maize_indices] <- vv(maize_mach_price, cv_maize_mach_price, length(maize_indices)) * Arable_area_AF #cost of machinery [€/ha]*area managed [ha]
  AF_maize_insurance_cost[maize_indices] <- vv(maize_insurance, cv_maize_insurance, length(maize_indices)) * Arable_area_AF #cost of insurance [€/ha]*area managed [ha]
  Maize_labour[maize_indices] <- vv(maize_labour, cv_maize_labour, length(maize_indices)) 
  AF_maize_labour_cost <- (Maize_labour + Maize_labour * (vv(extra_arable_time, var_cv, n_years)/100)) * Labour_costs * Arable_area_AF
  #Labour cost associated with maize cultivation. Total labour time is estimated to increase by 5-30% (extra_arable_time/100) due to more complicated navigation of machinery in AF system
  ...
  #Barley
  AF_barley_sowing_cost[barley_indices] <- vv(barley_seed_price, cv_barley_seed_price, length(barley_indices)) * Arable_area_AF #cost of seed [€/ha]*area managed [ha]
  AF_barley_fertilizer_cost[barley_indices] <- vv(barley_fert_price, cv_barley_fert_price, length(barley_indices)) * Arable_area_AF #cost of fertilizer [€/ha]*area managed [ha]
  AF_barley_pesticide_cost[barley_indices] <- vv(barley_cides_price, cv_barley_cides_price, length(barley_indices)) * Arable_area_AF #cost of pesticides [€/ha]*area managed [ha]
  AF_barley_machinery_cost[barley_indices] <- vv(barley_mach_price, cv_barley_mach_price, length(barley_indices)) * Arable_area_AF #cost of machinery [€/ha]*area managed [ha]
  AF_barley_insurance_cost[barley_indices] <- vv(barley_insurance, cv_barley_insurance, length(barley_indices)) * Arable_area_AF #cost of insurance [€/ha]*area managed [ha]
  Barley_labour[barley_indices] <- vv(barley_labour, cv_barley_labour, length(barley_indices)) 
  AF_barley_labour_cost <- (Barley_labour + Barley_labour * (vv(extra_arable_time, var_cv, n_years)/100)) * Labour_costs * Arable_area_AF #Labour cost associated with barley cultivation
  ...
  #Rapeseed
  AF_rapeseed_sowing_cost[rapeseed_indices] <- vv(rapeseed_seed_price, cv_rapeseed_seed_price, length(rapeseed_indices)) * Arable_area_AF #cost of seed [€/ha]*area managed [ha]
  AF_rapeseed_fertilizer_cost[rapeseed_indices] <- vv(rapeseed_fert_price, cv_rapeseed_fert_price, length(rapeseed_indices)) * Arable_area_AF #cost of fertilizer [€/ha]*area managed [ha]
  AF_rapeseed_pesticide_cost[rapeseed_indices] <- vv(rapeseed_cides_price, cv_rapeseed_cides_price, length(rapeseed_indices)) * Arable_area_AF #cost of pesticides [€/ha]*area managed [ha]
  AF_rapeseed_machinery_cost[rapeseed_indices] <- vv(rapeseed_mach_price, cv_rapeseed_mach_price, length(rapeseed_indices)) * Arable_area_AF #cost of machinery [€/ha]*area managed [ha]
  AF_rapeseed_insurance_cost[rapeseed_indices] <- vv(rapeseed_insurance, cv_rapeseed_insurance, length(rapeseed_indices)) * Arable_area_AF #cost of insurance [€/ha]*area managed [ha]
  Rapeseed_labour[rapeseed_indices] <- vv(rapeseed_labour, cv_rapeseed_labour, length(rapeseed_indices)) 
  AF_rapeseed_labour_cost <- (Rapeseed_labour + Rapeseed_labour * (vv(extra_arable_time, var_cv, n_years)/100)) * Labour_costs * Arable_area_AF #Labour cost associated with rapeseed cultivation
  ...
  AF_tot_apple_yield <- AF_apple_yield * num_trees
  #Calculate how many kg have table apple quality and can therefore be marketed at a higher price  
  Pc_table_apples <- vv(perc_table_apple, var_CV = var_cv, n_years)/100
  
  Table_apple_yield <- AF_tot_apple_yield * Pc_table_apples
  Juice_apple_yield <- AF_tot_apple_yield * (1-Pc_table_apples)
  
  #The benefits from table apples and juice apples are calculated by multiplying their yields by their respective prices  
  Table_apple_benefit <- vv(table_apple_price, var_cv, n_years) * Table_apple_yield
  Juice_apple_benefit <- vv(juice_apple_price, var_cv, n_years) * Juice_apple_yield
  
  AF_apple_benefit <- Table_apple_benefit + Juice_apple_benefit
  
  #Crop rotation in AF system
  AF_maize_yield[maize_indices] <- vv(maize_yield, cv_maize_yield, length(maize_indices)) * Arable_area_AF
  AF_maize_benefit <- vv(maize_price, cv_maize_price, n_years) * AF_maize_yield
  ...
  AF_barley_yield[barley_indices] <- vv(barley_yield, cv_barley_yield, length(barley_indices)) * Arable_area_AF
  AF_barley_benefit<- vv(barley_price, cv_barley_price, n_years) * AF_barley_yield
  
  AF_rapeseed_yield[rapeseed_indices] <- vv(rapeseed_yield, cv_rapeseed_yield, length(rapeseed_indices)) * Arable_area_AF
  AF_rapeseed_benefit <- vv(rapeseed_price, cv_rapeseed_price, n_years) * AF_rapeseed_yield
